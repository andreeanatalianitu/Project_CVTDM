...
standard.values = preProcess(train.dummy[,-c(2:6,8:11,37)], method=c("center", "scale"))
train.st[,-c(2:6,8:11,37)] <- predict(standard.values, train.dummy[,-c(2:6,8:11,37)])
valid.st[,-c(2:6,8:11,37)] <- predict(standard.values, valid.dummy[,-c(2:6,8:11,37)])
test.st[,-c(2:6,8:11,37)] <- predict(standard.values, test.dummy[,-c(2:6,8:11,37)])
...
knn1.test <- knn(train=train.st[,-37], test=test.st[,-37], cl=train.st[,37], k=1)
knn10.test <- knn(train=train.st[,-37], test=test.st[,-37], cl=train.st[,37], k=10)

knn1.acc <- data.frame(df=c("valid","test"))
knn1.acc$Balanced_Accuracy <- c(confusionMatrix(knn1, valid.st[,37], positive = "1")$byClass["Balanced Accuracy"], confusionMatrix(knn1.test, test.st[,37], positive = "1")$byClass["Balanced Accuracy"])
knn1.acc$F1 <- c(confusionMatrix(knn1, valid.st[,37], positive = "1")$byClass["F1"], confusionMatrix(knn1.test, test.st[,37], positive = "1")$byClass["F1"])

knn.acc <- data.frame(df=c("valid k=1","valid k=10","test k=1","test k=10"))
knn.acc$Balanced_Accuracy <- c(confusionMatrix(knn1, valid.st[,37], positive = "1")$byClass["Balanced Accuracy"], confusionMatrix(knn10, valid.st[,37], positive = "1")$byClass["Balanced Accuracy"], confusionMatrix(knn1.test, test.st[,37], positive = "1")$byClass["Balanced Accuracy"], confusionMatrix(knn10.test, test.st[,37], positive = "1")$byClass["Balanced Accuracy"])
...
"valid"=c(nb.valid.balanced.acc, lr.valid.balanced.acc, cm.da.valid$byClass["F1"], confusionMatrix(knn10, valid.st[,16], positive = "1")$byClass["F1"] , cm.nn1.valid$byClass["F1"], CT_cm_valid$byClass["F1"], boost_valid.cm$byClass["F1"], bag_valid.cm$byClass["F1"], rf_valid.cm$byClass["F1"]),

"test"=c(nb.test.balanced.acc, lr.test.balanced.acc, cm.da.test$byClass["F1"], confusionMatrix(knn10.test, test.st[,16], positive = "1")$byClass["F1"], cm.nn1.test$byClass["F1"], CT_cm_test$byClass["F1"], boost_test.cm$byClass["F1"], bag_test.cm$byClass["F1"], rf_test.cm$byClass["F1"]))
F1
```
